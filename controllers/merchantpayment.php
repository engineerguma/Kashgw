...
class Merchantpayment extends Controller {
    ...
    function MakeDebitRequest() {
     ...
     //   $this->model->log->TraceLog('req_from_merchant',$worker.'Merchantpayment::MakeDebitRequest Headers: ' . var_export($this->model->getRequestHeaders(), true), 2);
    ...
    $general=array('status'=>400,
      ...
      echo json_encode($general);
            ...
            function USSDDebitRequest() {
                ...
                $postdata['request']='debit';
                $worker = "Thread_ID_".getmypid().rand(10000 ,99999)."::";
          //      $this->model->log->TraceLog('req_from_ussd',$worker.'Merchantpayment::USSDDebitRequest Headers: ' . var_export($this->model->getRequestHeaders(), true), 2);               
    ...
    function MakeCreditRequest() {
        ...
        $worker = "Thread_ID_".getmypid().rand(10000 ,99999)."::";
     //   $this->model->log->TraceLog('req_from_merchant',$worker.'Merchantpayment::MakeCreditRequest Headers: ' . var_export($this->model->getRequestHeaders(), true), 2);               
        $req =$this->model->AuthenticateAccess('req_from_merchant',$worker);         
