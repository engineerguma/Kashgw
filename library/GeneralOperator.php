...
class GeneralOperator extends Model {
    ...
    function OperatorHandler($post_data,$transaction,$log_name){
        //Check Whether the transaction Is Duplicate:

        $this->log->LogRequest($log_name,"GeneralOperator:  HandleOperatorResponse ". var_export($post_data,true),2);
         unset($post_data['fri']);
         unset($post_data['transaction_reference_number']);
        //Wrap Up Transaction Processing And Prepare & Send Service Provider Response:
        $this->CloseTransaction($log_name,$transaction[0],$post_data);
        ...
        if($transaction[0]['transaction_source']=='ussd'){
          ...
          $this->SendMerchantCompletedRequest($transaction[0],$log_name);
            }
          exit();
        }
         $this->SendMerchantCompletedRequest($transaction[0],$log_name);
    ...
    function ProcessOperatorRequest($transaction,$log_name) {
      ...
      $this->log->LogRequest($log_name,"GeneralOperator:  ProcessOperatorRequest  ". var_export($transaction,true),2);

        $routing = $this->GetOperatorRouting($transaction['operator_id'],$transaction['transaction_type']);
         //print_r($transaction);die();
         $this->log->LogRequest($log_name,"GeneralOperator:  ProcessOperatorRequest::GetOperatorRouting  ". var_export($routing,true),2);

        $xml = $this->WriteGeneralXMLFile($routing[0], $transaction,$log_name);

        $result = $this->SendByCURL($routing[0]['routing_url'], $xml,$log_name);
